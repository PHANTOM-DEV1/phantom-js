...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = phantom.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from wif", function () {
      var secretKey = phantom.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.phantom);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");
    ...
    it("should be deserialised correctly", function () {
      var deserialisedTx = phantom.crypto.fromBytes(phantom.crypto.getBytes(vt).toString("hex"));
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(phantom.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = phantom.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = phantom.crypto.verifySecondSignature(vt, phantom.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
