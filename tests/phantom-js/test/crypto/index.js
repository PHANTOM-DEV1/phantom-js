...
describe("different networks", function () {
  ...
  it("validate address on tesnet should be ok", function () {
    phantom.crypto.setNetworkVersion(0x52);
    phantom.crypto.getNetworkVersion().should.equal(0x52);
    var validate = phantom.crypto.validateAddress("a6fpb1BJZq4otWiVsBcuLG1ZGs5WsqqQtH");
    (validate).should.equal(true);
    phantom.crypto.setNetworkVersion(0x37);
    phantom.crypto.getNetworkVersion().should.equal(0x37);
  });
});

describe("delegate.js", function () {
  var delegate = phantom.delegate;
  ...
  describe("#createDelegate", function () {
    ...
    describe("returned delegate", function () {
      var keys = phantom.crypto.getKeys("secret");
      var secondKeys = phantom.crypto.getKeys("secret 2");
      ...
      it("should be second signed correctly", function () {
        var result = phantom.crypto.verifySecondSignature(trs, secondKeys.publicKey);
        (result).should.be.ok;
      });
