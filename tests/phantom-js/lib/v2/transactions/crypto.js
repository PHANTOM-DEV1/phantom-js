...
function getBytes(transaction) {
	...
	bb.writeByte(transaction.network); // phantom = 0x37, devnet = 0x30
...
function verify(transaction, network) {
	network = network || networks.phantom;
...
function getKeys(secret, network) {
	var ecpair = ECPair.fromSeed(secret, network || networks.phantom);
